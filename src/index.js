...
function adminShell(active, title, subtitle, body, session) {
  ...
  const primary = active === "/admin/add" ? `<a class="btn" href="/admin">返回列表</a>` : `<a class="btn primary" href="/admin/add"><span class="icon">+</span>添加账号</a>`;
...
async function dashboard(request, env, session, url) {
  ...
  const body = `${messageBlock(url)}<section class="kpi-grid"><div class="card kpi"><span>账号总数</span><b>${vehicles.length}</b></div><div class="card kpi"><span>分享链接</span><b>${active?.count || 0}</b></div><div class="card kpi"><span>标签数量</span><b>${categories.length}</b></div><div class="card kpi"><span>今日访问</span><b>${views?.count || 0}</b></div></section><section><div class="card toolbar"><input id="search" placeholder="搜索账号、标签、内容..."><select id="category-filter"><option value="">全部标签</option>${options}</select><span class="badge"><span id="visible-count">${vehicles.length}</span> 项</span></div><div class="card table-card"><div class="table-head"><h2>账号列表</h2><span class="sub">点击验证码数字可复制 · 每 30 秒自动更新</span></div><div class="table-wrap">${rows.length ? `<table><thead><tr><th>账号</th><th>标签</th><th>验证码</th><th>分享链接</th><th>操作</th></tr></thead><tbody id="members">${rows.join("")}</tbody></table>` : `<div class="empty"><strong>还没有账号</strong><br>点击右上角添加第一个 TOTP 账号。</div>`}</div></div></section>`;
  const content = adminShell("/admin", "账号与验证码", "管理所有账号、分享链接、标签和访问风险。", body, session);
...
async function addAccountPage(env, session, url) {
  const { results: categories } = await env.DB.prepare("SELECT * FROM categories ORDER BY name ASC").all();
  const options = categoryOptions(categories);
  const body = `${messageBlock(url)}<div class="layout-grid"><section class="card form-card"><h2>账号信息</h2><form method="post" action="/admin/vehicles"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><label>名称</label><input name="name" required maxlength="80" placeholder="Google"><label>账号</label><input name="account" maxlength="160" placeholder="520.szxus@gmail.com"><div class="form-split"><div><label>编号</label><input name="code" required maxlength="20" placeholder="001"></div><div><label>标签</label><select name="category">${options}</select></div></div><label>Base32 密钥</label><input name="secret" required autocomplete="off" placeholder="JBSWY3DPEHPK3PXP"><label>备注</label><textarea name="notes" maxlength="2000" placeholder="保存后生成分享链接。"></textarea><button class="primary" style="margin-top:14px"><span class="icon">+</span>保存并生成链接</button></form></section><aside class="card form-card"><h2>保存后动作</h2><p class="muted">保存后立即出现在首页账号列表，并生成可复制的分享链接。</p><p class="note">重置分享链接后，旧链接会立即失效。</p></aside></div>`;
  return response(layout("添加账号", adminShell("/admin/add", "添加账号", "保存后自动生成分享链接，密钥加密写入 D1。", body, session), session));
}

async function categoriesPage(env, session, url) {
  const [{ results: categories }, { results: counts }] = await Promise.all([
    env.DB.prepare("SELECT * FROM categories ORDER BY name ASC").all(),
    env.DB.prepare("SELECT category, COUNT(*) AS count FROM vehicles GROUP BY category").all(),
  ]);
  const countMap = new Map(counts.map((row) => [row.category, row.count]));
  const rows = categories.map((category) => `<div class="tag-row"><div class="member-title"><span class="tag-dot ${esc(category.color || "green")}"></span><span><strong>${esc(category.name)}</strong><span class="sub">${countMap.get(category.name) || 0} 个账号</span></span></div><span class="badge">${esc(category.color || "green")}</span></div>`).join("");
  const body = `${messageBlock(url)}<section class="kpi-grid"><div class="card kpi"><span>标签总数</span><b>${categories.length}</b></div><div class="card kpi"><span>最多账号</span><b>${esc(categories[0]?.name || "—")}</b></div><div class="card kpi"><span>未分类</span><b>${countMap.get("未分类") || 0}</b></div></section><div class="layout-grid"><section class="card table-card"><div class="table-head"><h2>标签列表</h2><span class="sub">可编辑名称、颜色和备注</span></div>${rows || `<div class="empty">暂无标签</div>`}</section><aside class="card form-card"><h2>新建标签</h2><form method="post" action="/admin/categories"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><label>标签名称</label><input name="name" required maxlength="80"><label>颜色</label><select name="color"><option value="green">绿色</option><option value="blue">蓝色</option><option value="orange">橙色</option><option value="purple">紫色</option><option value="gray">灰色</option></select><button class="primary" style="width:100%;margin-top:12px">添加标签</button></form></aside></div>`;
  return response(layout("标签分类", adminShell("/admin/categories", "标签分类", "用标签把账号按用途分组，首页筛选会同步使用这些标签。", body, session), session));
}

async function settingsPage(session, url) {
  const body = `${messageBlock(url)}<div class="layout-grid"><section class="card table-card"><div class="table-head"><h2>偏好设置</h2><span class="sub">修改后自动保存</span></div><div class="setting-row"><div><strong>点击验证码直接复制</strong><p class="muted">首页和分享页都启用</p></div><span class="badge neutral">已开启</span></div><div class="setting-row"><div><strong>重置链接二次确认</strong><p class="muted">避免误操作导致旧链接失效</p></div><span class="badge neutral">已开启</span></div><div class="setting-row"><div><strong>夜间模式</strong><p class="muted">右上角按钮快速切换</p></div><button class="btn" type="button" data-theme-toggle>切换模式</button></div></section><aside class="card form-card"><h2>安全建议</h2><p class="muted">管理员密码建议定期更新，分享链接只给可信成员使用。</p><form method="post" action="/logout"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><button class="danger" style="width:100%;margin-top:14px">退出登录</button></form></aside></div>`;
  return response(layout("系统设置", adminShell("/admin/settings", "系统设置", "配置登录、安全提示、复制行为和界面偏好。", body, session), session));
}
...
async function handle(request, env) {
  ...
  if (path === "/admin/add" && method === "GET") return addAccountPage(env, session, url);
  if (path === "/admin/categories" && method === "GET") return categoriesPage(env, session, url);
  if (path === "/admin/settings" && method === "GET") return settingsPage(session, url);
  ...
  if (path === "/admin/logs" && method === "GET") {
    ...
    const body = `<div class="card table-card"><div class="table-head"><h2>最近访问记录</h2><span class="sub">最多显示 300 条</span></div><div class="table-wrap">${rows ? `<table class="logs-table"><thead><tr><th>账号</th><th>IP</th><th>User-Agent</th><th>时间</th></tr></thead><tbody>${rows}</tbody></table>` : `<div class="empty">暂无访问记录</div>`}</div></div>`;
