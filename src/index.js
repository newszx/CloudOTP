...
function layout(title, content, session = null) {
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} · TOTP 看板</title><style>${CSS}${DESIGN_CSS}${REFERENCE_CSS}${LIGHT_CSS}</style></head><body>
...
async function categoryPage(env, session, url) {
  ...
  const rows = categories.map((category) => `<tr data-category-row data-name="${esc(category.name.toLowerCase())}"><td><strong>${esc(category.name)}</strong></td><td>${Number(countMap.get(category.name) || 0)} 个账号</td><td><span class="badge green">启用</span></td><td><div class="row-actions"><button type="button" class="danger" disabled>删除</button></div></td></tr>`).join("");
  ...
  const body = `${messageBlock(url)}<section class="category-head"><div class="category-title"><span class="page-icon">${ICON_CATEGORIES}</span><div><h2>分类管理</h2><p class="muted">管理账号分类，便于组织和查找账号</p></div></div><div class="category-tools"><input id="category-search" placeholder="搜索分类名称或备注"><button class="primary" type="button" data-open-category-modal><span class="icon">+</span>添加分类</button></div></section><div class="modal-backdrop" data-modal="category" hidden>${categoryForm}</div><section class="stats-grid"><div class="card kpi"><span>分类总数</span><b>${categories.length}</b></div><div class="card kpi"><span>启用分类</span><b>${categories.length}</b></div><div class="card kpi"><span>账号总数</span><b>${counts.reduce((sum, row) => sum + Number(row.total || 0), 0)}</b></div></section><section class="card table-card category-table"><div class="table-wrap"><table><thead><tr><th>分类名称</th><th>账号数量</th><th>状态</th><th>操作</th></tr></thead><tbody>${rows}</tbody></table></div></section>`;
